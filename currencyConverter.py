...
class CurrencyConverter(customtkinter.CTkFrame):  # Κλάση που υλοποιεί το widget μετατροπής νομισμάτων
    ...
    def __init__(self, master, mode=None, theme=None, sound_enabled=True, **kwargs): # Αρχικοποίηση της κλάσης με παραμέτρους master, theme και sound_enabled
        ...
        self.from_currency_menu = customtkinter.CTkComboBox(
            ...
            # Χρώμα περιγράμματος combobox, αν δεν υπάρχει ορισμένο χρώμα στο θέμα, χρησιμοποιούμε #eb7c16
        )
        ...
        self.to_currency_menu.grid(row=2, column=2, padx=(5, 20), pady=(0, 10),
                                   sticky="ew")  # Τοποθέτηση του combobox για επιλογή τελικού νομίσματος
        ...
        self.amount_label.grid(row=3, column=0, columnspan=4, pady=(30, 2),
                               sticky="s")  # Τοποθέτηση της ετικέτας "Ποσό"
        ...
        self.amount_entry.grid(row=4, column=0, columnspan=4, padx=80, pady=(0, 30),
                               sticky="ew")  # Τοποθέτηση του πεδίου εισαγωγής ποσού
        ...
        self.convert_button.grid(row=5, column=0, columnspan=4, padx=80, pady=10,
                                 sticky="ew")  # Τοποθέτηση του κουμπιού μετατροπής
        ...
        self.result_value_label.grid(row=6, column=1, columnspan=3, padx=(5, 20), pady=(5, 100),
                                     sticky="w")  # Τοποθέτηση της ετικέτας αποτελέσματος
        ...
        self.exchange_rate_label.grid(row=7, column=0, columnspan=4, padx=20, pady=(0, 20),
                                      sticky="ew")  # Τοποθέτηση της ετικέτας ισοτιμίας

        # Καλούμε το apply_theme εδώ για να εφαρμοστεί το αρχικό θέμα
        ...
        try:
            ...
            if rates and to_currency.upper() in rates:  # Έλεγχος αν οι ισοτιμίες υπάρχουν και αν το νόμισμα στο οποίο μετατρέπουμε είναι έγκυρο
                ...
                self.result_value_label.configure(
                    text=f"{result:.2f} {to_currency.upper()}")  # Ενημέρωση της ετικέτας αποτελέσματος με το αποτέλεσμα της μετατροπής
                # --------------------------------------------------
                rate_text = f"(1 {from_currency.upper()} = {conversion_rate:.4f} {to_currency.upper()})"  # Δημιουργία κειμένου για την ισοτιμία
                self.exchange_rate_label.configure(
                    text=rate_text)  # Ενημέρωση της ετικέτας ισοτιμίας με την ισοτιμία της μετατροπής
                # --------------------------------------------------
            else:
                self.result_value_label.configure(
                    text="Conversion rate not found")  # Αν δε βρεθεί η ισοτιμία, ενημερώνουμε την ετικέτα αποτελέσματος
                self.exchange_rate_label.configure(
                    text="")  # Αν δε βρεθεί η ισοτιμία, καθαρίζουμε και την ετικέτα ισοτιμίας

        except ValueError:  # Αν η είσοδος ποσού δεν είναι έγκυρη (π.χ. Δεν είναι αριθμός)
            self.result_value_label.configure(
                text="Invalid Amount")  # Ενημέρωση της ετικέτας αποτελέσματος με μήνυμα σφάλματος
            self.exchange_rate_label.configure(
                text="")  # Αν δεν είναι έγκυρη η είσοδος ποσού, καθαρίζουμε και την ετικέτα ισοτιμίας

        except requests.exceptions.RequestException as e:  # Αν υπάρχει σφάλμα κατά την κλήση στο API
            self.result_value_label.configure(
                text=f"Error fetching data: {e}")  # Ενημέρωση της ετικέτας αποτελέσματος με μήνυμα σφάλματος
            self.exchange_rate_label.configure(
                text="")  # Αν υπάρχει σφάλμα κατά την κλήση στο API, καθαρίζουμε και την ετικέτα ισοτιμίας
    ...
    def handle_key_input(self, key):  # Διαχείριση εισόδου από το πληκτρολόγιο
        if key == "\x08":  # Backspace
            self.amount_entry.delete(0, customtkinter.END)  # Διαγραφή του περιχομένου του πεδίου εισαγωγής ποσού
        elif key == "\r":  # Enter
    ...
    def apply_theme(self, theme_dict):
        """
        Εφαρμόζει το θέμα στα widgets του CurrencyConverter,
        ...
        if self.amount_label:
            self.amount_label.configure(text_color=theme_dict.get("label_text", "#ffffff"))
        if self.result_label:
            self.result_label.configure(text_color=theme_dict.get("label_text", "#ffffff"))
        if self.result_value_label:
            self.result_value_label.configure(text_color=theme_dict.get("display_text", "#00ff00"))
        if self.exchange_rate_label:
            self.exchange_rate_label.configure(text_color=theme_dict.get("placeholder_text", "#aaaaaa"))

        # Comboboxes
        # Διαβάζουμε τα χρώματα από το theme_dict μία φορά
        ...
        combobox_border_color = theme_dict.get("menu_button_bg", "#eb7c16")  # Χρησιμοποιούμε το ίδιο χρώμα για border

        if self.from_currency_menu:
            self.from_currency_menu.configure(
                ...
                border_color=combobox_border_color  # Εφαρμογή χρώματος περιγράμματος
            )
        if self.to_currency_menu:
            self.to_currency_menu.configure(
                button_color=combobox_bg_color,
                dropdown_fg_color=combobox_dropdown_fg_color,
                dropdown_text_color=combobox_text_color,
                text_color=combobox_text_color,
        ...
        # Entry field
        entry_fg_color = theme_dict.get("entry_fg", "#ffffff")
        entry_text_color = theme_dict.get("text_input", "#000000")
        placeholder_text_color = theme_dict.get("placeholder_text", "#BEBEBE")
        ...
        # Convert button
        button_fg_color = theme_dict.get("special_button_fg", "#eb7c16")
        button_text_color = theme_dict.get("op_button_text", "#ffffff")
        button_hover_color = theme_dict.get("op_hover", "#5e5e5e")
...
if __name__ == "__main__":
    ...
    converter = CurrencyConverter(app, theme=theme, sound_enabled=True)
